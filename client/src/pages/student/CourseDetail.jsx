...
const CourseDetail = () => {
	...
	const { data: userData, isLoading: userIsLoading, error: userError } = useLoadUserQuery();
		...
		<div className="space-y-5 my-16">
			<div className="bg-[#2D2F31] text-white">
				<div className="max-w-7xl mx-auto py-8 px-4 md:px-8 flex flex-col gap-2">
					...
					{
						(user?.role === "instructor" && user?._id === course.creator._id) ? (<><p>Students enrolled: {course?.enrolledStudents.length}</p></>) : (<></>)
					}
				</div>
			</div>
